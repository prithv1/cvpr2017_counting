...
cmd:option('-list_dir', 'data/imagelists', 'Directory containing the imagelists corresponding to each split')
cmd:option('-feature_dim', 4096, 'Dimensions of the input feature vectors')
cmd:option('-num_classes', 80, 'Number of output classes')
...
cmd:option('-train_gt', 'train_count.mat', 'Path to train split ground truth count')
cmd:option('-val_gt', 'val_count.mat', 'Path to val split ground truth count')
cmd:option('-test_gt', 'test_count.mat', 'Path to test split ground truth count')
...
cmd:option('-gpu_flag', 1, 'Whether to use GPU or not')
cmd:option('-gpu_id', 1, 'GPU-ID to use')
cmd:option('-cudnn_flag', 1, 'Whether to use CuDNN or not')
...
local train_utils = train_utils(data_dir, list_dir, disc, opt.feature_dim, opt.num_classes, opt.num_epochs, opt.optimizer, opt.learning_rate, opt.weight_decay, opt.loss)
